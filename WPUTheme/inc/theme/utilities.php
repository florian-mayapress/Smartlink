...
function wputh_get_share_methods($post, $title = false, $permalink = false, $image = false) {
    ...
    if (has_post_thumbnail($post->ID)) {
        if (function_exists('wputhumb_get_thumbnail_url')) {
            $_image = rawurlencode(wputhumb_get_thumbnail_url('thumbnail', $post->ID));
        }
        else {
    ...
    $_methods = array(
        'email' => array(
            'name' => 'Email',
            'url' => 'mailto:mail@mail.com?subject=' . rawurlencode($_title) . '&body=' . rawurlencode($_title . ' ' . $_permalink)
        ) ,
        'facebook' => array(
            'name' => 'Facebook',
            'url' => 'https://www.facebook.com/sharer/sharer.php?u=' . rawurlencode($_permalink)
        ) ,
        'googleplus' => array(
            'name' => 'Google Plus',
            'url' => 'https://plus.google.com/share?url=' . rawurlencode($_permalink)
        ) ,
        'linkedin' => array(
            'name' => 'LinkedIn',
            'url' => 'https://www.linkedin.com/shareArticle?mini=true&url=' . rawurlencode($_permalink) . '&title=' . rawurlencode($_title) . '&summary=&source='
        ) ,
        'pinterest' => array(
            'name' => 'Pinterest',
            'url' => 'https://pinterest.com/pin/create/button/?url=' . rawurlencode($_permalink) . (!empty($_image) ? '&media=' . $_image : '') . '&description=' . rawurlencode($_title)
        ) ,
        'twitter' => array(
            'name' => 'Twitter',
            'url' => 'https://twitter.com/intent/tweet?text=' . rawurlencode($_twitter_text . ' ' . $_permalink) . rawurlencode($_via)
        ) ,
        'viadeo' => array(
            'name' => 'Viadeo',
            'url' => 'https://www.viadeo.com/shareit/share/?url' . rawurlencode($_permalink) . '&title=' . rawurlencode($_title) . ''
        ) ,
    );
